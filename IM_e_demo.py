...
class Environment(object):
    def __init__(self, ncol, nrow, subject, state_init='zeros', seed=0):
        ...
        #¥self.size       = [nrow, ncol]
        self.ruleorder1 = np.random.permutation(np.arange(1, 5))
        print(self.ruleorder1)
        #  c.f. GoL: 2, 3, 3
        self.rule = 1
        ...
        #print("Rule %d: "%self.rule, self.rule_param[self.rule-1])
        #/
        ...
        self.click_data = pd.DataFrame(columns=['Rule','Start','Stop','x','y'], index=pd.Index([0.], name='time'))

        self.check = np.vectorize(self._check)
        # ----- GUI -----
        # 画面初期化
        self.win = Tk()
        self.win.title('Demo[%d, %d, %d]' % (self.ruleorder1[0] ,self.ruleorder1[1] ,self.ruleorder1[2]))
        ...
        #/
        #- self.rule1btn = ttk.Checkbutton(self.frame2, text="Rule1", width=10, variable=self.v, onvalue=1, command=self.change_rule)
        # self.rule1btn.grid(row=1,column=1)
        # self.rule2btn = ttk.Checkbutton(self.frame2, text="Rule2", width=10, variable=self.v, onvalue=2, command=self.change_rule)
        # self.rule2btn.grid(row=1,column=2)
        # self.rule3btn = ttk.Checkbutton(self.frame2, text="Rule3", width=10, variable=self.v, onvalue=3, command=self.change_rule)
    ...
    def start_click(self, _):
        ...
        selapsed = time.time() - self.start_time
        self.click_data = self.click_data.append(pd.DataFrame({'Rule':"RULE%d" % self.rule,'Start': "Start"}, index=pd.Index([selapsed], name='time')))
    ...
    def canvas_click(self, x, y):
        ...
        self.click_data \
            = self.click_data.append(pd.DataFrame({'x': xx, 'y': yy}, index=pd.Index([elapsed], name='time')))
    ...
    def on_closing(self):
        ...
        self.click_data.to_csv('{0}_sub_{1}_demo_click.csv'.format(current_time, self.subject))  # クリック情報
        np.save('{0}_sub_{1}_demo_env.npy'.format(current_time, self.subject), self.state_npdata)  # 状態遷移
        self.state_data.to_csv('{0}_sub_{1}_demo_state.csv'.format(current_time, self.subject))  # 状態遷移
    ...
    def next_turn(self):
        ...
        #+
        self.state = self.state.reshape(self.size)
        #/
